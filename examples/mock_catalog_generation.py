...
print("Number of center galaxies: {}".format(len(r)))
...
center_clumps, flat_clumps, flats = gt.generate_clumps(np.append(r, r_rim), np.append(theta, theta_rim), np.append(phi, phi_rim))
...
plt.subplot(241)
plt.hist(gt.template_r, bins=int(np.sqrt(len(gt.template_r))), alpha=0.3, normed=True, label="Template")
plt.hist(r, bins=int(np.sqrt(len(r))), alpha=0.3, normed=True, label="BAO Centers")
plt.legend(loc=2)
plt.xlabel("r [Mpc]")

plt.subplot(242)
plt.hist(gt.template_r, bins=int(np.sqrt(len(gt.template_r))), alpha=0.3, normed=True, label="Template")
plt.hist(r_rim, bins=int(np.sqrt(len(r_rim))), alpha=0.3, normed=True, label="BAO rims")
plt.legend(loc=2)
plt.xlabel("r [Mpc]")

plt.subplot(243)
plt.hist(gt.template_r, bins=int(np.sqrt(len(gt.template_r))), alpha=0.3, normed=True, label="Template")
plt.hist(center_clumps[0], bins=int(np.sqrt(len(center_clumps[0]))), alpha=0.3, normed=True, label="Center clumps")
plt.legend(loc=2)
plt.xlabel("r [Mpc]")

plt.subplot(244)
plt.hist(gt.template_r, bins=int(np.sqrt(len(gt.template_r))), alpha=0.3, normed=True, label="Template")
plt.hist(flat_clumps[0], bins=int(np.sqrt(len(flat_clumps[0]))), alpha=0.3, normed=True, label="Flat clumps")
plt.legend(loc=2)
plt.xlabel("r [Mpc]")

plt.subplot(245)
plt.hist(gt.template_r, bins=int(np.sqrt(len(gt.template_r))), alpha=0.3, normed=True, label="Template")
plt.hist(flats[0], bins=int(np.sqrt(len(flats[0]))), alpha=0.3, normed=True, label="Flats")
plt.legend(loc=2)
plt.xlabel("r [Mpc]")

ax = plt.subplot(246)
ax.hist(theta, alpha=0.3, normed=True)
ax.hist(theta_rim, alpha=0.3, normed=True)
ax.hist(center_clumps[1], alpha=0.3, normed=True)
ax.hist(flat_clumps[1], alpha=0.3, normed=True)
ax.hist(flats[1], alpha=0.3, normed=True)
ax.set_xlabel(r'$\theta$ [deg]')
ax = plt.subplot(247)
ax.hist(phi, alpha=0.7, label="center galaxies", normed=True)
ax.hist(phi_rim, alpha=0.3, label="rim galaxies", normed=True)
ax.hist(center_clumps[2], alpha=0.3, label="center clumps", normed=True)
ax.hist(flat_clumps[2], alpha=0.3, label="flat clumps", normed=True)
ax.hist(flats[2], alpha=0.3, label="flat galaxies", normed=True)
box = ax.get_position()
...
all_rs     = np.append(r, np.append(r_rim, np.append(center_clumps[0], np.append(flat_clumps[0], flats[0]))))
all_zs     = gt.r2z(r=all_rs)
all_thetas = np.append(theta, np.append(theta_rim, np.append(center_clumps[1], np.append(flat_clumps[1], flats[1]))))
all_phis   = np.append(phi, np.append(phi_rim, np.append(center_clumps[2], np.append(flat_clumps[2], flats[2]))))
all_types  = np.append(np.full(len(r), 0),
                       np.append(np.full(len(r_rim), 1),
